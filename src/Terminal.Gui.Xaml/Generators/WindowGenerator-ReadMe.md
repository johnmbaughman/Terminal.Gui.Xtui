...
The `WindowGenerator` class is a Roslyn-based source generator that transforms XAML markup into C# code for Terminal.Gui applications. It uses the **Microsoft.CodeAnalysis.CSharp.SyntaxFactory** API to programmatically construct C# syntax trees, which are then converted into source code.
...
This enables developers to design UI in XAML while Terminal.Gui remains code-first, with the XAML being transformed into the imperative C# code that Terminal.Gui expects.
...
- **internal**: Only visible within the `Terminal.Gui.Xaml` assembly
...
public override StatementSyntax[] GenerateStatements(
    ElementNode node,           // The parsed XAML element
...
The `ElementNode` represents the parsed XAML structure:
```csharp
...
## Step 2: Setting Properties from XAML Attributes
...
For each attribute in the XAML element, we generate one assignment statement. This uses LINQ to transform the `Dictionary<string, string>` of attributes into syntax nodes.
...
**Note:** More complex generators might add multiple members (properties for XAML elements, methods for event handlers, etc.)
...
```csharp
namespace Xaml{public partial class MyWindow:Window{public MyWindow(){var label0=new Label();}}}
```
...
Let's trace the complete flow for our example XAML:
...
1. **CodeGenerator** (main generator) reads the XAML file
2. **XamlLoader** parses XAML into an `ElementNode` tree:
   ```
...
```csharp
namespace Xaml{public class MyWindow{}}
```
...
- Run only when XAML files change (incremental generation)
...
```
obj/Generated/Terminal.Gui.Xaml/Terminal.Gui.Xaml.CodeGenerator/MyWindow.g.cs
```
...
For XAML with `x:Name` attributes:
...
The generator transforms declarative XAML into imperative C# code, enabling a XAML-based UI design experience for Terminal.Gui while maintaining the framework's code-first nature.
